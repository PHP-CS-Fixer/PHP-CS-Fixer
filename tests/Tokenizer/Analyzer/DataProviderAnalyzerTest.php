...
final class DataProviderAnalyzerTest extends TestCase
{
    ...
    public static function provideGettingDataProviders80Cases(): iterable
    {
        ...
        yield 'with multiple DataProvider attributes' => [
            [
                new DataProviderAnalysis('provider1', 70, [21]),
                new DataProviderAnalysis('provider2', 84, [35]),
                new DataProviderAnalysis('provider3', 98, [48]),
            ],
            <<<'PHP'
                <?php
                class FooTest extends TestCase {
        ...
        yield 'with DataProvider attributes use in a tricky way' => [
            [
                new DataProviderAnalysis('provider1', 151, [60, 126]),
                new DataProviderAnalysis('provider2', 165, [84]),
                new DataProviderAnalysis('provider3', 179, [95]),
            ],
            <<<'PHP'
                <?php
                namespace N;
