 ...
 *
 * @phpstan-import-type _AutogeneratedInputConfiguration from \PhpCsFixer\Fixer\Operator\LongToShorthandOperatorFixer
 */
...
final class LongToShorthandOperatorFixerTest extends AbstractFixerTestCase
{
    ...
    {
        // A string offset cannot take an assign-op: PHP fails at runtime with
        // "Cannot use assign-op operators with string offsets". Whether an
        // offset belongs to a string or to an array cannot be told from the
        // source, so offsets are left alone by default.
        yield 'string offset is left alone by default' => [
            '<?php $text[0] = $text[0] & "\x7F";',
        ];

        yield 'offset on a property is left alone by default' => [
            '<?php $this->a[1] = $this->a[1] . "x";',
        ];

        yield 'offsets are shortened when opted in' => [
            '<?php $a[1] += 2;',
            '<?php $a[1] = $a[1] + 2;',
            ['skip_offset_targets' => false],
        ];

        yield 'nested offsets are shortened when opted in' => [
            '<?php $a[1][2] -= 852;',
            '<?php $a[1][2] = $a[1][2] - 852;',
            ['skip_offset_targets' => false],
        ];
