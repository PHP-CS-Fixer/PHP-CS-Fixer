...
use PhpCsFixer\Console\Command\NewVersionChecker;
use PHPUnit\Framework\TestCase;

/**
 * @internal
 *
 * @covers \PhpCsFixer\Console\Command\NewVersionChecker
 */
final class NewVersionCheckerTest extends TestCase
{
     ...
     * @var null|string
     */
    public static $fileGetContentsResult = null;

    private $githubResponse = <<<'JSON'
[
  {
    "name": "v2.4.1",
...
use PhpCsFixer\Tests\Console\Command\NewVersionCheckerTest;

function file_get_contents($filename, $useIncludePath = false, $context, $offset = 0, $maxlen = null)
{
    if (null !== NewVersionCheckerTest::$fileGetContentsResult) {
        $result = NewVersionCheckerTest::$fileGetContentsResult;
        NewVersionCheckerTest::$fileGetContentsResult = null;
...
PHP
    ...
    {
        self::$fileGetContentsResult = $this->githubResponse;

        $checker = new NewVersionChecker($currentVersion);
    ...
    /**
     * @param string|false $fileGetContentsResult
     * @param string       $expectedMessage
     *
     * @dataProvider getGithubRequestFailureCases
     */
    public function testGetLatestVersionWithGithubRequestFailure($fileGetContentsResult, $expectedMessage)
    {
        self::$fileGetContentsResult = $fileGetContentsResult;

        $checker = new NewVersionChecker('v1.2.3');

        $this->setExpectedExceptionRegExp('RuntimeException', '/^'.preg_quote($expectedMessage, '/').'/');
     ...
     * @dataProvider getGithubRequestFailureCases
     */
    public function testGetLatestVersionOfCurrentMajorWithGithubRequestFailure($fileGetContentsResult, $expectedMessage)
    {
        self::$fileGetContentsResult = $fileGetContentsResult;
        ...
        $checker->getLatestVersionOfCurrentMajor();
    }

    public function getGithubRequestFailureCases()
    {
        return array(
            array(
                false,
                'Failed to load tags at "https://api.github.com/repos/FriendsOfPHP/PHP-CS-Fixer/tags".',
            ),
            array(
                '<html><body>500 Internal Server Error</body></html>',
                'Failed to read response from "https://api.github.com/repos/FriendsOfPHP/PHP-CS-Fixer/tags" as JSON: Syntax error.',
            ),
        );
    }
}
