...
final class FixCommandTest extends TestCase
{
    ...
    public function testEmptyRulesBase64Value(): void
    {
        $this->expectException(
            InvalidConfigurationException::class
        );
        ...
        $this->doTestExecute(
            ['--rules-base64' => base64_encode('')]
        );
    }

    public function testInvalidBase64RulesValue(): void
    {
        $this->expectException(InvalidConfigurationException::class);
        $this->expectExceptionMessage('Invalid base64 encoding in rules-base64 option.');

        $this->doTestExecute(
            ['--rules-base64' => 'invalid-base64-string!@#']
        );
    }

    public function testRulesBase64WithJsonConfiguration(): void
    {
        $rules = ['array_syntax' => ['syntax' => 'short']];
        $base64Rules = base64_encode(json_encode($rules));

        $cmdTester = $this->doTestExecute([
            '--rules-base64' => $base64Rules,
            '--show-progress' => 'none',
        ]);
    ...
    public function testRulesBase64WithCommaDelimitedRules(): void
    {
        $rules = 'array_syntax,no_trailing_comma_in_singleline';
        $base64Rules = base64_encode($rules);
    ...
    public function testConflictBetweenRulesAndRulesBase64(): void
    {
        $this->expectException(InvalidConfigurationException::class);
        $this->expectExceptionMessage('Passing both `--rules` and `--rules-base64` options is not allowed.');

        $this->doTestExecute([
            '--rules' => 'array_syntax',
            '--rules-base64' => base64_encode('no_trailing_comma_in_singleline'),
        ]);
    }

    public function testConflictBetweenConfigAndRulesBase64(): void
    {
        $this->expectException(InvalidConfigurationException::class);
        $this->expectExceptionMessage('Passing both `--config` and (`--rules` or `--rules-base64`) options is not allowed.');

        $configFile = tempnam(sys_get_temp_dir(), 'php-cs-fixer-test-config-').'.php';
        file_put_contents($configFile, '<?php return new PhpCsFixer\Config();');

        try {
            $this->doTestExecute([
                '--config' => $configFile,
                '--rules-base64' => base64_encode('array_syntax'),
            ]);
        } finally {
            unlink($configFile);
        }
