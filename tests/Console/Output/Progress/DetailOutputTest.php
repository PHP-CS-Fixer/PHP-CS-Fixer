...
{
    /**
     * @param list<array{0: string, 1: int, 2?: array<string>}> $fileStatuses
     *
    ...
    public function testDotsProgressOutput(array $fileStatuses, string $expectedOutput): void
    {
        ...
        $this->foreachStatus(
            $fileStatuses,
            static function (\SplFileInfo $file, int $status, array $appliedFixers = []) use ($processOutput): void {
                $processOutput->onFixerFileProcessed(new FixerFileProcessedEvent($file, $status, $appliedFixers));
            }
        );
