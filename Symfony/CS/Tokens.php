...
class Tokens extends \SplFixedArray
{
     ...
     * @param  string|array $tokenB token element generated by token_get_all
     * @return bool
     */
    ...
    public static function isCast($token)
    {
        ...
        return static::isTokenGivenKind($token, $castTokens);
    }
    ...
    public static function isClassy($token)
    {
        ...
        return static::isTokenGivenKind($token, $classTokens);
    }
    ...
    public static function isComment($token)
    {
        ...
        return static::isTokenGivenKind($token, $commentTokens);
    }
     ...
     * @return bool
     */
    public static function isKeyword($token, $possibleKeywords = null)
    {
     ...
     * @return bool
     */
    public static function isTokenGivenKind($token, $possibleKinds)
    {
        return is_array($token) && (is_array($possibleKinds) ? in_array($token[0], $possibleKinds) : $token[0] === $possibleKinds);
    }
    ...
    public static function isWhitespace($token, array $opts = array())
    {
        ...
        return
            (is_string($token) && '' === trim($token, $whitespaces))
                ||
            (static::isTokenGivenKind($token, T_WHITESPACE) && '' === trim($token[1], $whitespaces))
        ;
    }
    ...
    public function getClassyElements()
    {
        ...
        foreach ($this as $index => $token) {
            ...
            if ('{' === $token || static::isTokenGivenKind($token, array(T_CURLY_OPEN, T_DOLLAR_OPEN_CURLY_BRACES, ))) {
                ++$curlyBracesLevel;
                continue;
            }
    ...
    public function grabAttribsBeforeToken($index, $tokenAttribsMap, $attribs)
    {
        while (true) {
            ...
            if (static::isTokenGivenKind($token, array(T_WHITESPACE, T_COMMENT, T_DOC_COMMENT, ))) {
                continue;
            }
