...
class Tokens extends \SplFixedArray
{
    ...
    public static function fromCode($code)
    {
        ...
        $collection->applyTransformators();
        $collection->changeCodeHash($codeHash);

        return $collection;
    }

    private static $transformators = array();

    private static function registerBuiltInTransformators()
    {
    ...
    public static function registerTransformator(TransformatorInterface $transformator)
    {
        self::$transformators[] = $transformator;

        $transformator->registerConstants();

        foreach ($transformator->getConstantDefinitions() as $value => $name) {
            Token::registerCustomToken($value, $name);
        }
    }

    public static function getTransformators()
    {
        self::registerBuiltInTransformators();

        return self::$transformators;
    }

    private function applyTransformators()
    {
        foreach (self::getTransformators() as $transformator) {
            $transformator->process($this);
        }
    }
