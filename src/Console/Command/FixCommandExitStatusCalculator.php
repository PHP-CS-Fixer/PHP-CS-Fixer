...
final class FixCommandExitStatusCalculator
{
     ...
     * @return int
     */
    public function calculate($isDryRun, $hasChangedFiles, $hasInvalidErrors, $hasExceptionErrors, $hasLintErrorsAfterFixing, $isRawFormat)
    {
        $exitStatus = 0;

        if ($isDryRun) {
            if ($hasChangedFiles && !$isRawFormat) {
