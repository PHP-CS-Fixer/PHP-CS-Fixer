...
final class WarningsDetector
{
    ...
    public function detectNonMonolithic(): void
    {
        if (filter_var(getenv('PHP_CS_FIXER_NON_MONOLITHIC'), \FILTER_VALIDATE_BOOL)) {
            $this->warnings[] = 'Touching non-monolithic files because `PHP_CS_FIXER_NON_MONOLITHIC` is set. Execution may be unstable - non-monolithic files are not officially supported.';
        }
    }
