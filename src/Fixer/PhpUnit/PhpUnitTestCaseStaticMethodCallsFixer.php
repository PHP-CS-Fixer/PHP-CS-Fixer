...
final class PhpUnitTestCaseStaticMethodCallsFixer extends AbstractPhpUnitFixer implements ConfigurableFixerInterface
{
     ...
     * @internal
     */
    public const SET_SYMFONY = '@Symfony';
    ...
    /**
     * Symfony WebTestCase assertion methods.
     *
     * @var array<string, true>
     */
    private const SYMFONY_METHODS = [
        'assertResponseIsSuccessful' => true,
        'assertResponseStatusCodeSame' => true,
        'assertResponseRedirects' => true,
        'assertResponseHasHeader' => true,
        'assertResponseNotHasHeader' => true,
        'assertResponseHeaderSame' => true,
        'assertResponseHeaderNotSame' => true,
        'assertResponseHasCookie' => true,
        'assertResponseNotHasCookie' => true,
        'assertResponseCookieValueSame' => true,
        'assertResponseFormatSame' => true,
        'assertEmailCount' => true,
        'assertQueuedEmailCount' => true,
        'assertEmailIsQueued' => true,
        'assertEmailIsNotQueued' => true,
        'assertEmailAttachmentCount' => true,
        'assertEmailAddressContains' => true,
        'assertEmailTextBodyContains' => true,
        'assertEmailTextBodyNotContains' => true,
        'assertEmailHtmlBodyContains' => true,
        'assertEmailHtmlBodyNotContains' => true,
        'assertEmailHasHeader' => true,
        'assertEmailNotHasHeader' => true,
        'assertEmailHeaderSame' => true,
        'assertEmailHeaderNotSame' => true,
        'assertSelectorExists' => true,
        'assertSelectorNotExists' => true,
        'assertSelectorTextContains' => true,
        'assertSelectorTextNotContains' => true,
        'assertSelectorTextSame' => true,
        'assertPageTitleSame' => true,
        'assertPageTitleContains' => true,
        'assertInputValueSame' => true,
        'assertInputValueNotSame' => true,
        'assertCheckboxChecked' => true,
        'assertCheckboxNotChecked' => true,
        'assertFormValue' => true,
        'assertNoFormValue' => true,
        'assertRequestAttributeValueSame' => true,
        'assertRouteSame' => true,
        'assertAnySelectorTextContains' => true,
        'assertAnySelectorTextNotContains' => true,
        'assertSelectorCount' => true,
        'assertSelectorWillExist' => true,
        'assertSelectorWillNotExist' => true,
        'assertSelectorAttributeWillContain' => true,
        'assertSelectorAttributeWillNotContain' => true,
    ];
    ...
    /**
     * Processed methods including custom assertions.
     *
     * @var array<string, true>
     */
    private array $processedMethods = [];
    ...
    public function getDefinition(): FixerDefinitionInterface
    {
        ...
        return new FixerDefinition(
            ...
            [
                ...
                new CodeSample(
                    $customAssertionSample,
                    ['assertions' => ['assertCustomThing']],
                ),
                new CodeSample(
                    $customAssertionSample,
                    ['assertions' => [self::SET_SYMFONY]],
                ),
                new CodeSample(
                    $customAssertionSample,
                    ['assertions' => [self::SET_SYMFONY, 'assertCustomThing']],
                ),
            ],
            null,
    ...
    protected function createConfigurationDefinition(): FixerConfigurationResolverInterface
    {
        return new FixerConfigurationResolver([
            (new FixerOptionBuilder('assertions', 'List of custom assertion method names or sets (`@Symfony`, `@all`) to be recognized by the fixer.'))
                ->setAllowedTypes(['string[]'])
                ->setAllowedValues([static function (array $value): bool {
                    $knownSets = [self::SET_ALL, self::SET_SYMFONY];

                    foreach ($value as $assertion) {
                        if ('' === trim($assertion) || trim($assertion) !== $assertion) {
                        ...
                        if (str_starts_with($assertion, '@') && !\in_array($assertion, $knownSets, true)) {
                            throw new InvalidOptionsException(
                                \sprintf(
                                    'Unknown set "%s", known sets are %s.',
                                    $assertion,
                                    Utils::naturalLanguageJoin($knownSets),
                                ),
                            );
                        }
                    }
    ...
    protected function configurePostNormalisation(): void
    {
        ...
        foreach (self::METHODS as $method => $unused) {
            $this->processedMethods[$method] = true;
        }

        foreach ($this->configuration['assertions'] as $assertion) {
            if (self::SET_ALL === $assertion) {
                // Add all predefined sets
                foreach (self::SYMFONY_METHODS as $method => $unused) {
                    $this->processedMethods[$method] = true;
                }
            } elseif (self::SET_SYMFONY === $assertion) {
                // Add Symfony-specific assertions
                foreach (self::SYMFONY_METHODS as $method => $unused) {
                    $this->processedMethods[$method] = true;
                }
            } elseif (!str_starts_with($assertion, '@')) {
                // Add custom assertion method
                $this->processedMethods[$assertion] = true;
            }
        }

        // Validate that methods in 'methods' config are recognized
        foreach ($this->configuration['methods'] as $method => $callType) {
            if (!isset($this->processedMethods[$method])) {
                throw new InvalidFixerConfigurationException(
                    $this->getName(),
                    \sprintf(
                        'Method "%s" is not recognized. Either it\'s not a PHPUnit method or it needs to be added to the "assertions" configuration option.',
                        $method,
                    ),
                );
            }
    ...
    protected function applyPhpUnitClassFix(Tokens $tokens, int $startIndex, int $endIndex): void
    {
        ...
        for ($index = $startIndex; $index < $endIndex; ++$index) {
            ...
            if (!$tokens[$index]->isGivenKind(\T_STRING) || !isset($this->processedMethods[$tokens[$index]->getContent()])) {
                continue;
            }
