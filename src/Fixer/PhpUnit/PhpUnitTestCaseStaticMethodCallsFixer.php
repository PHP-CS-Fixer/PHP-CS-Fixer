 ...
 *  call_type_for_former?: 'inherit'|'none'|'this',
 *  methods?: list<string>
 * }
...
final class PhpUnitTestCaseStaticMethodCallsFixer extends AbstractPhpUnitFixer implements ConfigurableFixerInterface
{
     ...
     * @internal
     */
    public const CALL_TYPE_INHERIT = 'inherit';

    /**
     * @internal
     */
    public const CALL_TYPE_NONE = 'none';

    /**
     * @var array<string, true>
     */
    private const CURRENT_STATIC_METHODS = [
        // Assert methods
        'anything' => true,
        'arrayHasKey' => true,
     ...
     * @var array<string, true>
     */
    private const FORMER_STATIC_METHODS = [
        'any' => true,
        'at' => true,
        'atLeast' => true,
        'atLeastOnce' => true,
     ...
     * @var array<string, true>
     */
    private const STATIC_METHODS = self::CURRENT_STATIC_METHODS + self::FORMER_STATIC_METHODS;
     ...
     * @var array<string, bool>
     */
    private const ALLOWED_VALUES_FOR_FORMER = [
        self::CALL_TYPE_THIS => true,
        self::CALL_TYPE_INHERIT => true,
    ...
    protected function createConfigurationDefinition(): FixerConfigurationResolverInterface
    {
        return new FixerConfigurationResolver([
            (new FixerOptionBuilder('call_type', 'The call type to use for referring to PHPUnit methods.'))
                ...
                ->setDefault('static')
                ->getOption(),
            (new FixerOptionBuilder('call_type_for_former', 'The call type to use for referring to formerly static PHPUnit methods.'))
                ->setAllowedTypes(['string'])
                ->setAllowedValues(array_keys(self::ALLOWED_VALUES_FOR_FORMER))
                ->setDefault('inherit')
                ->getOption(),
    ...
    protected function applyPhpUnitClassFix(Tokens $tokens, int $startIndex, int $endIndex): void
    {
        ...
        for ($index = $startIndex; $index < $endIndex; ++$index) {
            ...
            if ($tokens[$index]->isGivenKind(T_FUNCTION)) {
                ...
                $isMethodStatic = $attributes['static'];
            }

            $methodName = $tokens[$index]->getContent();

            if (!$tokens[$index]->isGivenKind(T_STRING) || !isset(self::STATIC_METHODS[$methodName])) {
                continue;
            }

            $callType = !isset(self::FORMER_STATIC_METHODS[$methodName]) || self::CALL_TYPE_INHERIT === $this->configuration['call_type_for_former']
                ? $this->configuration['call_type']
                : $this->configuration['call_type_for_former'];

            if (self::CALL_TYPE_NONE === $callType) {
                continue;
            }

            // do not change `self` to `this` in static methods
            if ($isMethodStatic && 'this' === $callType) {
                $index = $this->findEndOfNextBlock($tokens, $index);
