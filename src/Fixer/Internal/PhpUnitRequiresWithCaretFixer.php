 ...
 * @internal
 */
final class PhpUnitRequiresWithCaretFixer extends AbstractPhpUnitFixer implements InternalFixerInterface
{
    ...
    public function getDefinition(): FixerDefinitionInterface
    {
        return new FixerDefinition(
            'PHPUnit `@requires` annotation must have caret.',
            [
                new CodeSample(
                    <<<'PHP'
                        <?php
    ...
    private function fixDocComment(Tokens $tokens, int $index): void
    {
        ...
        $content = Preg::replace('/@requires PHP (\^\d)(?!\.)/', '@requires PHP $1.0', $content);
