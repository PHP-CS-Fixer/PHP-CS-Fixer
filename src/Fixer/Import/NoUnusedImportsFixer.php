 ...
 *  comments_match_case?: false|true,
 *  comments_search_annotations_only?: false|true
 * }
...
final class NoUnusedImportsFixer extends AbstractFixer implements ConfigurableFixerInterface
{
    ...
    protected function createConfigurationDefinition(): FixerConfigurationResolverInterface
    {
        return new FixerConfigurationResolver([
            (new FixerOptionBuilder('comments_match_case', 'Whether to treat comments as case-sensitive.'))
                ->setAllowedValues([true, false])
                ->setDefault(false)
                ->getOption(),
            (new FixerOptionBuilder('comments_search_annotations_only', 'Whether to analyze only annotations when considering comments.'))
                ->setAllowedValues([true, false])
                ->setDefault(false)
                ->getOption(),
        ]);
    ...
    private function isImportUsed(Tokens $tokens, NamespaceAnalysis $namespace, NamespaceUseAnalysis $import, array $ignoredIndices): bool
    {
        ...
        for ($index = $namespace->getScopeStartIndex(); $index <= $namespaceEndIndex; ++$index) {
            ...
            if ($token->isComment()) {
                ...
                if (true === $this->configuration['comments_search_annotations_only']) {
                    $annotationPattern = '(@\w+)\s*(\S*).*\R?';

                    if ($token->isGivenKind(T_DOC_COMMENT)) {
                        $annotationPattern = '\*\s'.$annotationPattern;
                    }

                    if (0 < Preg::matchAll('/'.$annotationPattern.'/m', $token->getContent(), $matches)) {
                        $content = array_merge(
                            array_diff($matches[1], ['@var', '@param', '@return']),
                            $matches[2],
                        );
                    }
                } else {
                ...
                $pattern = '/(?<![[:alnum:]\$])(?<!\\\)'.$import->getShortName().'(?![[:alnum:]])/';

                $pattern .= true === $this->configuration['comments_match_case']
                    ? ''
                    : 'i';
