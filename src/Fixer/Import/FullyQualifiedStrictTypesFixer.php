        ...
        foreach ($this->discoveredSymbols as $kind => $discoveredSymbols) {
                ...
                while (true) {
                    ...
                    if (!isset($discoveredFqcnByShortNameLower[$kind][$shortEndNameLower])) {
                        ...
                        if (('' === $namespaceName && !isset($useByShortNameLower[$shortStartNameLower]))
                            ...
                            $discoveredFqcnByShortNameLower[$kind][$shortEndNameLower] = $this->resolveSymbol($symbol->toString(), $kind, $uses, $namespaceName);
                    ...
                    if ('' === $namespaceName || $symbol->startsWith('\\') || !$symbol->containsAny('\\')) {
                        break;
                    }

                    $symbol = $symbol->beforeLast('\\');
                }
            }
            ...
            foreach ($discoveredFqcnByShortNameLower[$kind] ?? [] as $fqcn) {
                ...
                if (str_contains($shortenedName, '\\')) { // prevent importing non-namespaced names in global namespace
                    $shortEndName = (new ByteString($fqcn))->afterLast('\\')->toString();
