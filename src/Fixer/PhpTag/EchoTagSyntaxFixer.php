 ...
 *  format?: 'long'|'short',
 *  ignore_start_of_file?: bool,
 *  ignore_tag_without_closing_tag?: bool,
 *  long_function?: 'echo'|'print',
 *  shorten_simple_statements_only?: bool,
 *  shorten_tags_without_comments_only?: bool,
 * }
...
final class EchoTagSyntaxFixer extends AbstractFixer implements ConfigurableFixerInterface
{
    ...
    /** @internal  */
    public const OPTION_IGNORE_START_OF_FILE = 'ignore_start_of_file';

    /** @internal  */
    public const OPTION_IGNORE_TAG_WITHOUT_CLOSING_TAG = 'ignore_tag_without_closing_tag';

    /** @internal  */
    public const OPTION_SHORTEN_TAGS_WITHOUT_COMMENTS_ONLY = 'shorten_tags_without_comments_only';
    ...
    public function getDefinition(): FixerDefinitionInterface
    {
        ...
        return new FixerDefinition(
            ...
            [
                ...
                new CodeSample($sample, [self::OPTION_FORMAT => self::FORMAT_SHORT, self::OPTION_IGNORE_START_OF_FILE => true]),
                new CodeSample($sample, [self::OPTION_FORMAT => self::FORMAT_SHORT, self::OPTION_IGNORE_TAG_WITHOUT_CLOSING_TAG => true]),
                new CodeSample($sample, [self::OPTION_FORMAT => self::FORMAT_SHORT, self::OPTION_SHORTEN_TAGS_WITHOUT_COMMENTS_ONLY => true]),
            ],
            null,
        );
    }
    ...
    protected function createConfigurationDefinition(): FixerConfigurationResolverInterface
    {
        return new FixerConfigurationResolver([
            ...
            (new FixerOptionBuilder(self::OPTION_SHORTEN_SIMPLE_STATEMENTS_ONLY, 'Render short-echo tags only in case of simple code.'))
                ...
                ->setDefault(true)
                ->getOption(),
            (new FixerOptionBuilder(self::OPTION_IGNORE_START_OF_FILE, 'Ignore the tag at the first line of the file.'))
                ->setAllowedTypes(['bool'])
                ->setDefault(false)
                ->getOption(),
            (new FixerOptionBuilder(self::OPTION_IGNORE_TAG_WITHOUT_CLOSING_TAG, 'Ignore tags that do not have a closing ?> tag.'))
                ->setAllowedTypes(['bool'])
                ->setDefault(false)
                ->getOption(),
            (new FixerOptionBuilder(self::OPTION_SHORTEN_TAGS_WITHOUT_COMMENTS_ONLY, 'Render short-echo tags only if the tag doesn\'t contain a comment.'))
                ->setAllowedTypes(['bool'])
                ->setDefault(false)
                ->getOption(),
        ]);
    ...
    private function longToShort(Tokens $tokens): void
    {
        ...
        for ($index = 0; $index < $count; ++$index) {
            if ($this->configuration[self::OPTION_IGNORE_START_OF_FILE] && 0 === $index) {
                continue;
            }
            ...
            $closingToken = $tokens->getNextTokenOfKind($index, [[\T_CLOSE_TAG]]);
            if (null === $closingToken && true === $this->configuration[self::OPTION_IGNORE_TAG_WITHOUT_CLOSING_TAG]) {
                return;
            }

            if (true === $this->configuration[self::OPTION_SHORTEN_TAGS_WITHOUT_COMMENTS_ONLY]) {
                $comment = $tokens->getNextTokenOfKind($index, [[\T_COMMENT], [\T_DOC_COMMENT]]);
                if (null !== $comment && (null === $closingToken || $closingToken > $comment)) {
                    continue;
                }
            }
    ...
    private function shortToLong(Tokens $tokens): void
    {
        ...
        while (true) {
            ...
            if (true === $this->configuration[self::OPTION_IGNORE_TAG_WITHOUT_CLOSING_TAG] && null === $tokens->getNextTokenOfKind($index, [[\T_CLOSE_TAG]])) {
                return;
            }
