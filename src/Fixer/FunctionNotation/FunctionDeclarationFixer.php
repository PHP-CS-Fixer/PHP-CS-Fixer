            ...
            [
                ...
                new CodeSample(
                    <<<'PHP'
                        ...
                        PHP,
                    ['closure_use_spacing' => self::SPACING_NONE],
                ),
            ],
        );
    }
    ...
    protected function applyFix(\SplFileInfo $file, Tokens $tokens): void
    {
        ...
        for ($index = $tokens->count() - 1; $index >= 0; --$index) {
            ...
            if ($afterParenthesisToken->isGivenKind(CT::T_USE_LAMBDA)) {
                ...
                if (self::SPACING_NONE === $this->configuration['closure_use_spacing']) {
                    // optionally remove whitespace after `use`
                    // eg: `use ($bar)` => `use($bar)`
                    if ($tokens[$afterParenthesisIndex + 1]->isWhitespace()) {
                        $tokens->clearAt($afterParenthesisIndex + 1);
                    }
                } else {
    ...
    protected function createConfigurationDefinition(): FixerConfigurationResolverInterface
    {
        return new FixerConfigurationResolver([
            ...
            (new FixerOptionBuilder('closure_fn_spacing', 'Spacing to use before open parenthesis for short arrow functions.'))
                ...
                ->setAllowedValues(self::SUPPORTED_SPACINGS)
                ->getOption(),
            (new FixerOptionBuilder('closure_use_spacing', 'Spacing to use before open parenthesis for closure `use` clauses.'))
                ->setDefault(self::SPACING_ONE)
                ->setAllowedValues(self::SUPPORTED_SPACINGS)
                ->getOption(),
