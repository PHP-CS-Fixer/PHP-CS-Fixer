...
final class PhpdocTagNoNamedArgumentsFixer extends AbstractFixer implements ConfigurableFixerInterface, WhitespacesAwareFixerInterface
{
    ...
    public function createConfigurationDefinition(): FixerConfigurationResolverInterface
    {
        return new FixerConfigurationResolver([
            (new FixerOptionBuilder('description', 'Description of the tag.'))
                ...
                ->setDefault('')
                ->getOption(),
            (new FixerOptionBuilder('fix_attribute_classes', 'Whether to fix attribute classes.'))
    ...
    protected function applyFix(\SplFileInfo $file, Tokens $tokens): void
    {
        for ($index = $tokens->count() - 1; $index > 0; --$index) {
            ...
            if (!$this->configuration['fix_attribute_classes'] && self::isAttributeClass($tokens, $prevIndex)) {
                continue;
            }
    ...
    private function ensureIsDocBlockWithNoNameArgumentsTag(Tokens $tokens, int $index): void
    {
        ...
        if (null === $ensureIsDocBlockWithTagNoNameArguments) {
            $ensureIsDocBlockWithTagNoNameArguments = \Closure::bind(
                static function (WhitespacesFixerConfig $whitespacesConfig, Tokens $tokens, int $index, bool $fixInternal): void {
                    $phpUnitInternalClassFixer = new PhpUnitInternalClassFixer();
                    $phpUnitInternalClassFixer->setWhitespacesConfig($whitespacesConfig);
                    $phpUnitInternalClassFixer->ensureIsDocBlockWithAnnotation(
                        $tokens,
                        $index,
                        'no-named-arguments',
                        $fixInternal ? ['no-named-arguments'] : ['internal', 'no-named-arguments'],
                        []
                    );
                },
                null,
        ...
        $ensureIsDocBlockWithTagNoNameArguments($this->whitespacesConfig, $tokens, $index, $this->configuration['fix_internal']);
    }
}
