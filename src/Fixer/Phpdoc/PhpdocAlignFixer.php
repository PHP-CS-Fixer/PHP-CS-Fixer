 ...
 *  align?: 'left'|'left_multiline'|'vertical',
...
final class PhpdocAlignFixer extends AbstractFixer implements ConfigurableFixerInterface, WhitespacesAwareFixerInterface
{
     ...
     * @internal
     */
    public const ALIGN_LEFT_MULTILINE = 'left_multiline';
    ...
    protected function createConfigurationDefinition(): FixerConfigurationResolverInterface
    {
        $allowPositiveIntegers = static function ($value) {
            ...
            foreach ($spacings as $spacing => $val) {
                if (\is_int($val) && $val <= 0) {
                    if (self::MULTILINE_DEFAULT_SPACING_KEY === $spacing) {
                        if ($val < 0) {
                            throw new InvalidOptionsException('The option "spacing" is invalid. Minimum _multiline spacing must be zero.');
                        }
    ...
    private function leftMultilineSpacing(): int
    {
        return (\is_int($this->spacing))
            ? self::MULTILINE_DEFAULT_SPACING
            : ($this->spacing[self::MULTILINE_DEFAULT_SPACING_KEY] ?? self::MULTILINE_DEFAULT_SPACING);
    }
    ...
    {
        if (self::ALIGN_VERTICAL === $this->align) {
            return 0;
        }

        if (self::ALIGN_LEFT_MULTILINE === $this->align) {
            return $this->leftMultilineSpacing();
        }
