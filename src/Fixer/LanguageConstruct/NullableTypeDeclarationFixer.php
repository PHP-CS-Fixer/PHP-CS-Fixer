 ...
 *  union_null_position?: 'any'|'first'|'last',
 * }
...
final class NullableTypeDeclarationFixer extends AbstractFixer implements ConfigurableFixerInterface
{
    ...
    private const OPTION_UNION_NULL_POSITION_ANY = 'any'; // Default - do not reorder existing unions
    private const OPTION_UNION_NULL_POSITION_FIRST = 'first';
    private const OPTION_UNION_NULL_POSITION_LAST = 'last';

    private array $candidateTokenKind;
        ...
        return new FixerDefinition(
            ...
            [
                ...
                new VersionSpecificCodeSample(
                    ...
                    new VersionSpecification(8_00_00),
                    ['syntax' => self::OPTION_SYNTAX_UNION, 'union_null_position' => self::OPTION_UNION_NULL_POSITION_LAST]
                ),
                new VersionSpecificCodeSample(
                    '<?php
class ValueObject
...
',
    ...
    {
        return \PHP_VERSION_ID >= 8_00_00 && $tokens->isAnyTokenKindsFound($this->candidateTokenKind);
    }
    ...
    {
        $mainToken = self::OPTION_SYNTAX_QUESTION_MARK === $this->configuration['syntax']
            ? CT::T_TYPE_ALTERNATION // `|` -> `?`
            : CT::T_NULLABLE_TYPE;
        $this->candidateTokenKind = [$mainToken]; // `?` -> `|`

        if (self::OPTION_SYNTAX_UNION === $this->configuration['syntax']
            && self::OPTION_UNION_NULL_POSITION_ANY !== $this->configuration['union_null_position']
        ) {
            $this->candidateTokenKind[] = CT::T_TYPE_ALTERNATION;
        }
    }
    ...
    private function normalizeNullableType(Tokens $tokens, TypeAnalysis $typeAnalysis): void
    {
        ...
        if ($isQuestionMarkSyntax) {
            ...
            $normalizedType = $this->convertToExplicitUnionType($type, $this->configuration['union_null_position']);
            $normalizedTypeAsString = implode('|', $normalizedType);
        }
     ...
     * @return list<string>
     */
    private function convertToExplicitUnionType(string $type, string $nullPosition = self::OPTION_UNION_NULL_POSITION_ANY): array
    {
        if (str_contains($type, '|')) {
            // no need to convert; already fixed
            if (self::OPTION_UNION_NULL_POSITION_ANY === $nullPosition) {
                return [$type];
            }

            // no need to convert if correctly placed
            if (self::OPTION_UNION_NULL_POSITION_FIRST === $nullPosition && str_starts_with($type, 'null|')) {
                return [$type];
            }

            if (self::OPTION_UNION_NULL_POSITION_LAST === $nullPosition && str_ends_with($type, '|null')) {
                return [$type];
            }

            // reverse order and return
            return array_reverse(explode('|', $type));
        }

        if (self::OPTION_UNION_NULL_POSITION_LAST === $nullPosition) {
