...
',
    ...
    // FIXME Risky when refined PHP const in namespace, we need something similar like `isGlobalFunctionCall` for const; `isGlobalConstAccess`
    ...
    private function fixConstant(Tokens  $tokens, int $constIndex): void
    {
        ...
        if (!$tokens[$index]->isGivenKind(T_STRING)) {
            return; // constant already has a type, skip it FIXME T_CONST T_STRING T_STRING vs T_CONST T_STRING '=' check
        }
        ...
        if (
            ...
            && $tokens[$tokens->getNextMeaningfulToken($endIndex)]->equals(';')
        ) {
        ...
        if ($this->isMagicClassReferenceWithoutOperation($tokens, $valueIndex)) {
    ...
    private function fixSimpleStringConstant(
        ...
        assert($tokens[$valueIndex]->isGivenKind(T_STRING));
    ...
    private function getTypeForStringToken(Token $token): ?string
    {
        ...
        if ($this->isPredefinedConstStr($content)) {
            $type = 'string';
    ...
    private function isMagicClassReferenceWithoutOperation(
        Tokens $tokens,
        int $valueIndex
    ): bool {
