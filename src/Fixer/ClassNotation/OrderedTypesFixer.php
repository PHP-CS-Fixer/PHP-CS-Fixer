 ...
 *  case_sensitive?: bool,
 *  false_adjustment?: 'always_first'|'always_last'|'none',
...
final class OrderedTypesFixer extends AbstractFixer implements ConfigurableFixerInterface
{
    ...
    protected function createConfigurationDefinition(): FixerConfigurationResolverInterface
    {
        return new FixerConfigurationResolver([
            ...
            (new FixerOptionBuilder('null_adjustment', 'Forces the position of `null` (overrides `sort_algorithm`).'))
                ...
                ->getOption(),
            (new FixerOptionBuilder('false_adjustment', 'Forces the position of `false` (overrides `sort_algorithm`). When both `false_adjustment` and `null_adjustment` are set to the same value, `null` is sorted further out (e.g. `int|false|null` rather than `int|null|false`).'))
                ->setAllowedValues(['always_first', 'always_last', 'none'])
                ->setDefault('none')
                ->getOption(),
    ...
    private function runTypesThroughSortingAlgorithm(array $types): array
    {
        ...
        usort($types, function ($a, $b) use ($normalizeType): int {
            ...
            foreach ([['null', 'null_adjustment'], ['false', 'false_adjustment']] as [$typeToAdjust, $configField]) {
                if ('none' !== $this->configuration[$configField]) {
                    if ($typeToAdjust === $lowerCaseA && $typeToAdjust !== $lowerCaseB) {
                        return 'always_last' === $this->configuration[$configField] ? 1 : -1;
                    }
