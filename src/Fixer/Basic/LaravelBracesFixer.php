 ...
 */
final class LaravelBracesFixer extends AbstractProxyFixer implements WhitespacesAwareFixerInterface
{
    /**
     * @internal
     */
...
'
     ...
     * Must run after ClassAttributesSeparationFixer, EmptyLoopBodyFixer, LineEndingFixer, NoAlternativeSyntaxFixer, NoEmptyStatementFixer, NoUselessElseFixer, SingleLineThrowFixer, SingleTraitInsertPerStatementFixer.
     */
    ...
    private function configureFixer(): void
    {
        $this->configuration = [
            'allow_single_line_anonymous_class_with_empty_body' => true,
            'allow_single_line_closure' => false,
            'position_after_functions_and_oop_constructs' => self::LINE_NEXT,
            'position_after_control_structures' => self::LINE_SAME,
            'position_after_anonymous_constructs' => self::LINE_SAME,
            'position_after_anonymous_class' => self::LINE_NEXT,
        ];

        $this->getControlStructureContinuationPositionFixer()->configure([
            'position' => ControlStructureContinuationPositionFixer::SAME_LINE,
        ]);
    }
        ...
        for ($index = 0, $limit = \count($tokens); $index < $limit; ++$index) {
            ...
            if ($token->isGivenKind($classyTokens) && !$tokensAnalyzer->isAnonymousClass($index)) {
                ...
                || (
                    ...
                    || (
                        self::LINE_NEXT === $this->configuration['position_after_anonymous_class']
                        && $token->isGivenKind(T_CLASS) && $tokensAnalyzer->isAnonymousClass($index)
                    )
                )
            ) {
