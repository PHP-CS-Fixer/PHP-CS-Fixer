...
final class TokensAnalyzer
{
    ...
    public function isConstantInvocation(int $index): bool
    {
        ...
        if ($this->tokens[$prevIndex]->isGivenKind([T_AS, T_CLASS, T_CONST, T_DOUBLE_COLON, T_FUNCTION, T_GOTO, CT::T_GROUP_IMPORT_BRACE_OPEN, T_INTERFACE, T_TRAIT, CT::T_TYPE_COLON])
            || $this->tokens[$prevIndex]->isObjectOperator()
            || $this->tokens[$prevIndex]->getContent() === '|' // union type Foo|Bar
        ) {
            return false;
        }
