...
final class FullyQualifiedNameAnalyzer
{
    ...
    private function getFullyQualifiedNameWithPossiblyLeadingSlash(string $name, int $indexInNamespace, int $importType): string
    {
        ...
        $startIndex = $namespaceAnalysis->getStartIndex();

        $this->namespaceUseAnalyses[$startIndex] ??= (new NamespaceUsesAnalyzer())->getDeclarationsInNamespace($this->tokens, $namespaceAnalysis);
        \assert(isset($this->namespaceUseAnalyses[$startIndex]));

        $declarations = [];
        foreach ($this->namespaceUseAnalyses[$startIndex] as $namespaceUseAnalysis) {
