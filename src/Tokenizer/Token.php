...
final class Token
{
    ...
    private ?string $ignoredRules = null;
    ...
    /**
     * Empty string means ignore all rules.
     */
    public function ignoreRules(string $rules = ''): void
    {
        $rules = trim($rules);
        $this->ignoredRules = '' === $rules ? 'all' : $rules;
    }
    ...
    public function isIgnoredFor(AbstractFixer $fixer): bool
    {
        if (null === $this->ignoredRules) {
            return false;
        }
        if ('all' === $this->ignoredRules) {
            return true;
        }

        return str_contains($this->ignoredRules, $fixer->getName());
    }
