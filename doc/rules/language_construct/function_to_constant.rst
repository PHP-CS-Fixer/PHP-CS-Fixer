...
@Symfony:risky
  Using the ``@Symfony:risky`` rule set will enable the ``function_to_constant`` rule with the default config.

@PhpCsFixer:risky
  Using the ``@PhpCsFixer:risky`` rule set will enable the ``function_to_constant`` rule with the default config.
